...
class Player(Sprite):
    def __init__(self,num, startx, starty,step,health,movement_speed,attacking_speed,attacking_range,defending_range,blocking_time,points):
        ...
        self.jump_image = pygame.image.load("assets/p"+str(num)+"_jump.png")
        self.jump_image = pygame.image.load("assets/p"+str(num)+"_jump.png")
        self.jump_imageD = pygame.image.load("assets/p"+str(num)+"_jumpD.png")
        self.jump_imageG = pygame.image.load("assets/p"+str(num)+"_jumpG.png")
        self.attack_image = pygame.image.load("assets/p"+str(num)+"_attack.png")
        self.bloc_image = pygame.image.load("assets/p"+str(num)+"_block.png")
        self.num = num
        self.walk_cycle = [pygame.image.load(f"assets/p"+str(num)+"_walk01.png") for i in range(1,12)]
        ...
        #*60 pour repondre a la distance en pixels
        self.attacking_range = attacking_range*60
        self.defending_range = defending_range
        #si on a 3frame a defendre et que le fps est a 12 donc faut tenir 4s
    ...
    def update(self, sol,player2,clock,count):
        ...
        # Mouvement attack
            ...
            self.on_att=True
            #print(str(Sprite.get_position(player2)[0])+" -z- "+str(Sprite.get_position(self)[0])+" -z- "+str(self.attacking_range) +" -z- "+ str(player2.on_def)+" -z- "+ str(player2.on_att))
            if(Sprite.get_position(player2)[0]-self.attacking_range <= Sprite.get_position(self)[0] and not player2.on_def and not player2.on_att):
            ...
            elif Sprite.get_position(player2)[0]-self.attacking_range <= Sprite.get_position(self)[0] and player2.on_def and not player2.on_att :  
        ...
        if key[pygame.K_o]and self.num==2:
            self.image = self.attack_image
            self.on_att=True
            #print(str(Sprite.get_position(player2)[0])+" -o- "+str(Sprite.get_position(self)[0])+" -o- "+str(self.attacking_range) +" -o- "+ str(player2.on_def)+" -o- "+ str(player2.on_att))
            if(Sprite.get_position(self)[0]-self.attacking_range <= Sprite.get_position(player2)[0]  and not player2.on_def and not player2.on_att):
                LOAD_Music('audio/Coup.mp3')
                Play_music(1)
                self.points+=1
                player2.health-=10
                self.rect.center = [self.startx, self.starty]
                player2.rect.center = [player2.startx, player2.starty]
            elif Sprite.get_position(self)[0]-self.attacking_range <= Sprite.get_position(player2)[0]  and player2.on_def and not player2.on_att :  
                LOAD_Music('audio/bloc.mp3')
                Play_music(1)
            else:
                LOAD_Music('audio/CoupVide.mp3')
